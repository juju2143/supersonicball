...
function love.update(dt)
	...
	elseif state == "game" then
		...
		--[[if joystick ~= nil then
			ball.body:applyForce(400.*joystick:getAxis(1), 0)
			if touchnorm then
				ball.body:applyForce(0, 5000.*joystick:getAxis(2))
			end
		end]]
		if love.system.getOS() == "Android" then
			for index = 1, love.touch.getTouchCount() do
				local id, x, y, pressure = love.touch.getTouch(index)
				ball.body:applyForce(400.*(x-0.5), 0)
				if touchnorm then
					ball.body:applyForce(0, 5000.*(y-0.5))
				end
			end
		end
